...
import type { IntervalValue, Tranche } from './types.variables'; // Assurez-vous d'importer les types
...
import { XCircleIcon } from '@heroicons/react/24/solid'; // Pour les messages d'erreur
...
const IntervalEditor: React.FC<IntervalEditorProps> = ({ value, onChange, className }) => {
  ...
  const currentBase = value?.base ?? "";
  const currentTranches = value?.tranches ?? [];

  const handleBaseChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    onChange({ ...value, base: (e.target.value) });//onChange({ ...value, base: Number(e.target.value) });
  };
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div>
        ...
        <input
          type="text"
          id="interval-base"
          value={typeof value?.base === "number" ? String(value.base) : value.base ?? ""}
          //value={String(value?.base ?? "")}
          onChange={(e) => {
            const newBase = e.target.value;
            onChange({
              ...value,
              base: newBase, // 👈 string ou nombre : laissé libre
            });
          }}
